...
namespace Shmup
{
    class ParticleSprite : Sprite
    {
        ...
        public ParticleSprite(Texture2D newTxr, Vector2 newPos) : base(newTxr, newPos)
        {
            ...
            particleColour = new Color((float)(rando.NextDouble() / 2 + 0.5), (float)(rando.NextDouble() / 3 + 0.5), 1f);
        ...
        {
            _spriteBatch.Draw(spriteTexture, new Rectangle((int)spritePos.X, (int)spritePos.Y,(int)(spriteTexture.Width *(currentLife / maxLife) *2), (int)(spriteTexture.Height*(currentLife /maxLife) *2)), particleColour);
        }
    }
}
